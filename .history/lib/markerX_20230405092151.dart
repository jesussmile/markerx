...
class _MarkerLayerXState extends State<MarkerLayerX> {
  ...
  void initState() {
    ...
    putMarkers(bounds, markers);

    // visibleMarkers = _getVisibleMarkers();
  }

  Future<void> putMarkers(LatLngBounds bounds, List<MarkerX> markers) async {
    markerQuadtree = MarkerQuadtree(bounds, 500); // adjust capacity as needed
  ...
  Widget build(BuildContext context) {
    ...
    if (newBound != markerQuadtree.bounds) {
      markerQuadtree =
          MarkerQuadtree(newBound!, 500); // adjust capacity as needed

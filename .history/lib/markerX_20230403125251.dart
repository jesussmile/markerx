...
class MarkerLayerX extends StatelessWidget {
  ...
  Set<MarkerX> _getVisibleMarkers(FlutterMapState map) {
    ...
    for (final marker in markers) {
      ...
      if (cached == null) {
        cachedValues[marker] = {
          'pxPoint': map.project(marker.point),
          'portion': CustomPoint<num>(marker.width - marker.anchor.left,
              marker.height - marker.anchor.top),
        };
      }

      final sw = CustomPoint<double>(
          cached!['pxPoint']!.x + cached['portion']!.x, //- marker.anchor.right,
          cached['pxPoint']!.y + cached['portion']!.y // - marker.anchor.bottom,
          );
      final ne = CustomPoint<double>(
        cached['pxPoint']!.x - marker.anchor.left,
        cached['pxPoint']!.y - marker.anchor.top,
      );

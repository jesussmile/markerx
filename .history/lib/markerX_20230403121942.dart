...
class MarkerLayerX extends StatelessWidget {
  ...
  List<MarkerX> _getVisibleMarkers(FlutterMapState map) {
    ...
    final cachedValues = <MarkerX, Map<String, CustomPoint<double>>>{};

    for (final marker in markers) {
      final cached = cachedValues[marker] ?? {};
      cached['pxPoint'] ??= map.project(marker.point) as CustomPoint<double>;
      cached['rightPortion'] ??=
          (marker.width - marker.anchor.left) as CustomPoint<double>;

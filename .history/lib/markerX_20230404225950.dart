...
class _MarkerLayerXState extends State<MarkerLayerX> {
  ...
  void initState() {
    ...
    markerWidgetsCache = _buildMarkerWidgets(visibleMarkers);
  }

  List<Widget> _buildMarkerWidgets(List<MarkerX> markers) {
  ...
  Widget build(BuildContext context) {
    ...
    if (newZoom != currentZoom) {
      ...
      visibleMarkers = _getVisibleMarkers();
      if (newZoom == currentZoom) {
        // new zoom is the same as current zoom, use cache
        return Stack(
          children: markerWidgetsCache,
        );
      } else {
        // new zoom is different, recalculate widgets and update cache
        markerWidgetsCache = _buildMarkerWidgets(visibleMarkers);
      }
    }
    final markerWidgets =
        markerWidgetsCache; // use cache if new zoom is the same
    return Stack(
